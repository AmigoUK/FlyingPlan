...
def profile():
    if request.method == "POST":
        ...
        current_user.a2_cofc_number = request.form.get("a2_cofc_number", "").strip() or None
        gvc_level = request.form.get("gvc_level", "").strip()
        current_user.gvc_level = gvc_level if gvc_level in ('GVC', 'RPC_L1', 'RPC_L2', 'RPC_L3', 'RPC_L4') else None
        current_user.gvc_cert_number = request.form.get("gvc_cert_number", "").strip() or None
        oa_type = request.form.get("oa_type", "").strip()
        current_user.oa_type = oa_type if oa_type in ('PDRA_01', 'FULL_SORA') else None
        current_user.oa_reference = request.form.get("oa_reference", "").strip() or None
